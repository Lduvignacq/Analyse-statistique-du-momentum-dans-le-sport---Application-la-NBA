...
def fetch_team_scores(team_id: int, team_name: str) -> list[dict]:
    """
    ...
    for i, row in df_games.iterrows():
        ...
        try:
            ...
            if score_home.empty or score_visitor.empty:
                print(f"    ⚠️ pb " + str(gid))
                continue

            final_home    = int(score_home.iloc[-1])
            final_visitor = int(score_visitor.iloc[-1])
            margin        = final_home - final_visitor
            diff          = margin * sign

            if sign != 1:
                final_home, final_visitor = final_visitor, final_home
...
if __name__ == "__main__":
    ...
    for idx, team in enumerate(all_teams):
        ...
        records = fetch_team_scores(team_id, team_name)
